...
window.handleSelfReportUserSelect = function() {
  ...
  if(match) {
    ...
    if(srSelectedUser) {
      ...
      if(filteredCourses.length === 0) { 
         if(warnText) { warnText.textContent = `⚠️ ท่านไม่มีประวัติการอบรมในปีงบประมาณ ${activeYear} จึงไม่ต้องรายงานผลในรอบนี้`; warnText.classList.remove('hidden'); }
  ...
  if(warnText) { warnText.textContent = '⚠️ ไม่พบประวัติการอบรมของชื่อนี้ กรุณาตรวจสอบการสะกดคำ'; warnText.classList.remove('hidden'); }
  if(formContainer) formContainer.classList.add('hidden');
};
...
window.renderSrForms = function() {
  ...
  if(dynamicForms) dynamicForms.innerHTML = html; 
};
