  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.users && parsed.users.length > 0) {
        publicUsersList = parsed.users;
        globalSettings.activeReportYear = parsed.activeReportYear || '2569';
        globalSettings.adminPin = parsed.adminPin || '336699';
...
async function fetchPublicData() {
  ...
  try {
    ...
    if (result && result.status === 'success') {
      ...
      try {
        localStorage.setItem('SPORTS_HR_PUBLIC_USERS', JSON.stringify(result.data));
      } catch (e) {}
