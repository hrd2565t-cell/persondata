...
document.addEventListener('DOMContentLoaded', () => {
  ...
  setupOTPInputs();
  populateProvinces();
  switchPage('report'); // ตั้งหน้าแรกเป็นหน้ารายงาน
...
// 📌 ฟังก์ชันอัปเดตสถานะ (Inline Edit)
window.updatePersonnelStatus = async function(uid, newStatus, selectElement) {
  ...
  try {
    ...
    if (result.status === 'success') {
      ...
      // ดึงข้อมูลมาอัปเดตกราฟเบื้องหลัง
...
// 📌 ฟังก์ชันล้างข้อมูลหน้าค้นหารายงาน
...
window.openMatrixReport = function() {
  if (!globalFiltersMaster) return; matrixAvailableYears = [...globalFiltersMaster.years].sort((a, b) => a - b); if(matrixAvailableYears.length === 0) { alert("ไม่พบข้อมูลปีการศึกษาในระบบ"); return; }
  const startSelect = document.getElementById('matrixStartYear'); const endSelect = document.getElementById('matrixEndYear'); startSelect.innerHTML = matrixAvailableYears.map(y => `<option value="${y}">${y}</option>`).join(''); endSelect.innerHTML = matrixAvailableYears.map(y => `<option value="${y}">${y}</option>`).join(''); document.getElementById('matrixModal').classList.remove('hidden'); applyMatrixFilter('all');
}

window.applyMatrixFilter = function(type) {
  if (matrixAvailableYears.length === 0) return; const minYear = matrixAvailableYears[0]; const maxYear = matrixAvailableYears[matrixAvailableYears.length - 1]; let startYear, endYear;
  if (type === '5') { startYear = Math.max(minYear, maxYear - 4); endYear = maxYear; } else if (type === '10') { startYear = Math.max(minYear, maxYear - 9); endYear = maxYear; } else if (type === 'all') { startYear = minYear; endYear = maxYear; } else if (type === 'custom') { startYear = parseInt(document.getElementById('matrixStartYear').value); endYear = parseInt(document.getElementById('matrixEndYear').value); if (startYear > endYear) { alert('⚠️ ปีเริ่มต้นต้องไม่มากกว่าปีสิ้นสุด'); return; } }
  document.getElementById('matrixStartYear').value = startYear; document.getElementById('matrixEndYear').value = endYear; buildMatrixTable(startYear, endYear);
}
...
function buildMatrixTable(startYear, endYear) {
  ...
  const filteredYears = matrixAvailableYears.filter(y => y >= startYear && y <= endYear); const courses = globalFiltersMaster.courses; const courseCounts = globalFiltersMaster.courseYearCounts || {}; const catMap = globalFiltersMaster.courseCategoryMap || {};
  ...
  let html = `<table class="w-full text-sm border-collapse border border-slate-800 text-slate-800 mt-4"><thead><tr><th rowspan="2" class="border border-slate-800 p-3 bg-slate-100 w-1/4">หลักสูตร</th><th colspan="${filteredYears.length}" class="border border-slate-800 p-2 bg-slate-100 text-center">ปีที่อบรม (จำนวนบุคลากรกีฬา)</th><th rowspan="2" class="border border-slate-800 p-3 bg-slate-100 text-center w-20">รวม</th></tr><tr>${filteredYears.map(y => `<th class="border border-slate-800 p-2 text-center bg-slate-50 font-bold">${y}</th>`).join('')}</tr></thead><tbody>`;
  ...
  for (const [category, courseList] of Object.entries(groupedCourses)) {
    ...
    courseList.forEach(course => {
      ...
      if (rowTotal > 0 || filteredYears.length === matrixAvailableYears.length) { grandTotal += rowTotal; html += `<tr>${rowHtml}<td class="border border-slate-800 p-2 text-center font-bold">${rowTotal}</td></tr>`; } else { html += `<tr>${rowHtml}<td class="border border-slate-800 p-2 text-center font-bold text-slate-400">0</td></tr>`; }
    });
  }
...
window.exportMatrixToExcel = function() { const table = document.querySelector('#matrixTableContainer table'); if(!table) return alert('ไม่พบข้อมูลตาราง กรุณาลองใหม่อีกครั้ง'); const clonedTable = table.cloneNode(true); const cells = clonedTable.querySelectorAll('td.bg-gray-400'); cells.forEach(cell => cell.textContent = ''); const wb = XLSX.utils.table_to_book(clonedTable, {sheet: "Matrix_Report"}); XLSX.writeFile(wb, "รายงานสรุปตารางไขว้_Matrix.xlsx"); }
...
function renderTablePage() {
  const tbody = document.getElementById('tableBody'); const paginationInfo = document.getElementById('tablePaginationInfo');
  ...
  const totalItems = currentFilteredData.length; const totalPages = Math.ceil(totalItems / itemsPerPage); const startIndex = (currentPage - 1) * itemsPerPage; const endIndex = Math.min(startIndex + itemsPerPage, totalItems); const pageData = currentFilteredData.slice(startIndex, endIndex);
  ...
  if (paginationInfo) paginationInfo.innerHTML = `แสดงรายการที่ <span class="font-bold text-slate-800 mx-1">${startIndex + 1} - ${endIndex}</span> จากทั้งหมด <span class="font-bold text-slate-800 mx-1">${totalItems}</span> รายการ`; renderPaginationNav(totalPages);
}
...
function renderTimeline(relations, years, courses) {
  const container = document.getElementById('timelineCardsContainer'); if (!container) return;
  ...
  container.innerHTML = courses.map((course, idx) => {
    ...
    if (activeYears.length === 0) return ''; const firstYear = activeYears[0]; const lastYear = activeYears[activeYears.length - 1]; let missingYears = []; for (let y = firstYear; y <= lastYear; y++) { if (!activeYearsMap[y]) missingYears.push(y); }
    return `<div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow space-y-5"><div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-slate-100 pb-4"><div><span class="text-xs font-semibold text-blue-600 uppercase tracking-wider">หลักสูตรที่ #${idx + 1}</span><h3 class="text-lg font-bold text-slate-800 mt-0.5">${course}</h3></div><div class="flex flex-wrap items-center gap-2"><span class="bg-slate-100 text-slate-700 text-xs font-semibold px-3 py-1.5 rounded-lg">ช่วงเวลา: <span class="text-blue-600">${firstYear} - ${lastYear}</span></span><span class="bg-blue-50 text-blue-700 text-xs font-semibold px-3 py-1.5 rounded-lg border border-blue-100">จัดทั้งหมด ${activeYears.length} ปี</span></div></div><div><p class="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2.5">✅ ปีที่มีการเปิดอบรม:</p><div class="flex flex-wrap gap-2">${activeYears.map(y => `<div class="flex items-center gap-1.5 bg-blue-600 text-white text-xs font-bold px-3.5 py-1.5 rounded-xl shadow-sm"><svg class="w-3.5 h-3.5 text-blue-200" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.5 12.75l6 6 9-13.5" /></svg>ปี ${y}</div>`).join('')}</div></div>${missingYears.length > 0 ? `<div class="pt-3 border-t border-slate-100"><p class="text-xs font-semibold text-amber-600 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">⚠️ ปีที่เว้นช่วงการจัด (Gap):</p><div class="flex flex-wrap gap-2">${missingYears.map(my => `<span class="bg-amber-50 border border-amber-200 text-amber-700 text-xs font-semibold px-3 py-1 rounded-lg">ปี ${my} (งดจัด)</span>`).join('')}</div></div>` : `<div class="pt-3 border-t border-slate-100 text-xs text-emerald-600 font-semibold flex items-center gap-1.5"><span>✨ จัดอบรมต่อเนื่องทุกปีโดยไม่มีช่วงว่าง</span></div>`}</div>`;
  }).join('');
}
...
function formatThaiName(rawPrefix, rawName) {
  ...
  if (prefix === "น.ส.") prefix = "นางสาว"; name = name.split(/\s+/).join(' ');
...
function isSmartMatch(importName, existingName) {
  ...
  if (impKeywords.length > 0 && extKeywords.length > 0) { if (impKeywords.every(kw => noSpaceExt.includes(kw)) || extKeywords.every(kw => noSpaceImp.includes(kw))) return true; }
  return false;
}
...
function processExcelFile(file, inputElement) {
  ...
  reader.onload = function(event) {
    try {
      const data = new Uint8Array(event.target.result); const workbook = XLSX.read(data, {type: 'array'}); const jsonRows = XLSX.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]], { defval: "" });
      ...
      let cleanedRows = jsonRows.map((row, index) => {
        let formatted = formatThaiName(row['คำนำหน้า'], row['ชื่อ-นามสกุล']); let matchedExisting = null; let matchType = 'new'; 
        let smartFound = cachedPersonnelData.find(p => isSmartMatch(formatted.fullName, p.fullName));
        if (smartFound) { matchedExisting = smartFound; matchType = 'exact'; } else { for (let p of cachedPersonnelData) { let sim = calculateSimilarity(p.fullName, formatted.fullName); if (sim >= 0.75 && sim < 1.0) { matchedExisting = p; matchType = 'fuzzy'; break; } } }
        return { originalIndex: index, 'คำนำหน้า': formatted.prefix, 'ชื่อ-นามสกุล': formatted.fullName, 'กลุ่มหน่วยงาน': row['กลุ่มหน่วยงาน'] || '', 'หน่วยงาน': String(row['หน่วยงาน'] || '').replace(/\s+/g, ' ').trim(), 'สถานะ': row['สถานะ'] || 'ปฏิบัติงาน', 'ชื่อหลักสูตร': row['ชื่อหลักสูตร'] || '', 'ปีที่อบรม': row['ปีที่อบรม'] || '', matchType: matchType, matchedUser: matchedExisting, actionType: matchType === 'exact' ? 'merge' : 'auto' };
      });
      pendingImportData = cleanedRows; showPreviewSection();
    } catch (error) { alert("❌ เกิดข้อผิดพลาดในการอ่านไฟล์"); } if(inputElement) inputElement.value = ''; 
  };
  reader.readAsArrayBuffer(file);
}

window.submitSingleEntry = function() {
  const pPrefix = document.getElementById('singlePrefix').value; const pName = document.getElementById('singleFullName').value; const pGroup = document.getElementById('singleGroup').value; const pAgency = document.getElementById('singleAgency').value; const pCourse = document.getElementById('singleCourse').value; const pYear = document.getElementById('singleYear').value;
  if(!pName || !pAgency) { alert('⚠️ กรุณากรอก ชื่อ-นามสกุล และ หน่วยงาน ให้ครบถ้วน'); return; }

  let formatted = formatThaiName(pPrefix, pName); let matchedExisting = null; let matchType = 'new'; 
  let smartFound = cachedPersonnelData.find(p => isSmartMatch(formatted.fullName, p.fullName));
  if (smartFound) { matchedExisting = smartFound; matchType = 'exact'; } else { for (let p of cachedPersonnelData) { let sim = calculateSimilarity(p.fullName, formatted.fullName); if (sim >= 0.75 && sim < 1.0) { matchedExisting = p; matchType = 'fuzzy'; break; } } }

  pendingImportData = [{ originalIndex: 0, 'คำนำหน้า': formatted.prefix, 'ชื่อ-นามสกุล': formatted.fullName, 'กลุ่มหน่วยงาน': pGroup, 'หน่วยงาน': pAgency, 'สถานะ': 'ปฏิบัติงาน', 'ชื่อหลักสูตร': pCourse, 'ปีที่อบรม': pYear, matchType: matchType, matchedUser: matchedExisting, actionType: matchType === 'exact' ? 'merge' : 'auto' }];
  document.getElementById('singlePrefix').value = ''; document.getElementById('singleFullName').value = ''; document.getElementById('singleGroup').value = ''; document.getElementById('singleAgency').value = ''; document.getElementById('singleCourse').value = ''; document.getElementById('singleYear').value = ''; showPreviewSection();
}

function showPreviewSection() { document.getElementById('importUploadSection').classList.add('hidden'); document.getElementById('importPreviewSection').classList.remove('hidden'); currentPreviewPage = 1; renderPreviewTablePage(); }
...
function renderPreviewTablePage() {
  ...
  const totalItems = pendingImportData.length; const totalPages = Math.ceil(totalItems / previewItemsPerPage); const startIndex = (currentPreviewPage - 1) * previewItemsPerPage; const endIndex = Math.min(startIndex + previewItemsPerPage, totalItems); const pageData = pendingImportData.slice(startIndex, endIndex);
  ...
  tbody.innerHTML = pageData.map((row, idx) => {
    let badgeHtml = `<span class="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2.5 py-1 rounded-full text-xs font-semibold">✨ บุคคลใหม่</span>`; let targetUidVal = '';
    if (row.matchType === 'exact') { badgeHtml = `<span class="bg-blue-50 text-blue-700 border border-blue-200 px-2.5 py-1 rounded-full text-xs font-semibold">🔄 อัปเดตคนเดิม (${row.matchedUser.fullName})</span>`; targetUidVal = row.matchedUser.uid; } else if (row.matchType === 'fuzzy') { badgeHtml = `<div class="space-y-1.5"><span class="inline-block bg-amber-50 text-amber-800 border border-amber-300 px-2.5 py-0.5 rounded text-[11px] font-semibold">⚠️ ชื่อคล้าย: ${row.matchedUser.fullName}</span><select onchange="updateImportAction(${startIndex + idx}, this.value)" class="w-full text-xs bg-slate-50 border border-slate-300 rounded p-1.5 outline-none font-medium text-slate-700"><option value="auto" ${row.actionType === 'auto' ? 'selected' : ''}>-- กรุณาเลือก --</option><option value="merge" ${row.actionType === 'merge' ? 'selected' : ''}>🔗 รวมประวัติคนเดิม</option><option value="new" ${row.actionType === 'new' ? 'selected' : ''}>➕ สร้างใหม่แยก</option></select></div>`; targetUidVal = row.matchedUser.uid; }
    return `<tr class="hover:bg-slate-50 align-top border-b border-slate-100"><td class="px-4 py-3.5 border-r border-slate-100 text-center font-mono text-xs text-slate-400">${startIndex + idx + 1}</td><td class="px-4 py-3.5 border-r border-slate-100 font-medium text-slate-800">${row['คำนำหน้า']} ${row['ชื่อ-นามสกุล']}</td><td class="px-4 py-3.5 border-r border-slate-100 truncate max-w-[180px]">${row['หน่วยงาน'] || '-'}</td><td class="px-4 py-3.5 border-r border-slate-100 text-center"><span class="bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-xs">${row['สถานะ']}</span></td><td class="px-4 py-3.5 border-r border-slate-100">${row['ชื่อหลักสูตร'] || '-'} <span class="text-xs text-slate-400">(${row['ปีที่อบรม'] || '-'})</span></td><td class="px-4 py-3.5 text-center"><input type="hidden" id="targetUid_${startIndex + idx}" value="${targetUidVal}">${badgeHtml}</td></tr>`;
  }).join('');
  if (previewInfo) previewInfo.innerHTML = `แสดงรายการที่ <span class="font-bold text-slate-800 mx-1">${startIndex + 1} - ${endIndex}</span> จากทั้งหมด <span class="font-bold text-slate-800 mx-1">${totalItems}</span> รายการ`; renderPreviewPaginationNav(totalPages);
}
...
window.viewProfile = function(uid) {
  ...
  document.getElementById('profileAgency').textContent = `${person.agency} (${person.status})`; document.getElementById('profileGroup').textContent = person.group || 'ไม่ระบุกลุ่ม'; 

  const timelineEl = document.getElementById('profileTrainings');
  if (person.trainings && person.trainings.length > 0) { const sortedTrainings = person.trainings.sort((a, b) => b.year - a.year); timelineEl.innerHTML = sortedTrainings.map(t => `<li class="relative pl-6 pb-4 border-l-2 border-slate-200 last:border-0 last:pb-0"><div class="absolute w-3 h-3 bg-blue-500 rounded-full -left-[7px] top-1.5 ring-4 ring-white shadow-sm"></div><p class="text-sm font-bold text-slate-800">${t.course}</p><p class="text-xs text-slate-500 mt-0.5">ปีการศึกษา: ${t.year}</p></li>`).join(''); } else { timelineEl.innerHTML = `<li class="text-sm text-slate-500 pl-4">ยังไม่มีประวัติการอบรม</li>`; }
  
  const dutyEl = document.getElementById('profileDuties');
  if (person.duties && person.duties.length > 0) {
    dutyEl.innerHTML = person.duties.map(d => `<li class="bg-white p-3.5 rounded-xl border border-slate-200 flex flex-col gap-1 shadow-sm"><div class="flex justify-between items-start"><span class="text-sm font-bold text-slate-800">${d.sport}</span><span class="text-[10px] font-bold bg-slate-100 text-slate-600 px-2 py-0.5 rounded">ปี ${d.year || '-'}</span></div><span class="text-xs text-blue-600 font-semibold">${d.role}</span><span class="text-xs text-slate-500 flex items-center gap-1 mt-1"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"></path></svg> ${d.event || 'ไม่ระบุชื่องาน'}</span></li>`).join('');
  ...
  if (person.evals && person.evals.length > 0) { evalEl.innerHTML = person.evals.map(e => `<div class="bg-white p-3.5 rounded-xl border border-slate-200 text-sm text-slate-700 italic shadow-sm">"${e.feedback}"</div>`).join(''); } else { evalEl.innerHTML = `<div class="text-sm text-slate-500">ยังไม่มีข้อเสนอแนะ</div>`; }
...
window.closeProfile = function() { currentActiveUid = null; const backdrop = document.getElementById('slideOverBackdrop'); const panel = document.getElementById('slideOverPanel'); backdrop.classList.remove('opacity-100'); backdrop.classList.add('opacity-0'); panel.classList.remove('translate-x-0'); panel.classList.add('translate-x-full'); setTimeout(() => { document.getElementById('slideOver').classList.add('hidden'); }, 300); }
...
window.submitDuty = async function() {
  if (!currentActiveUid) return; const sport = document.getElementById('inputDutySport').value.trim(); const role = document.getElementById('inputDutyRole').value.trim(); const event = document.getElementById('inputDutyEvent').value.trim(); const year = document.getElementById('inputDutyYear').value.trim();
  ...
  try {
    ...
    const result = await response.json();
    if(result.status === 'success') { alert('✅ บันทึกสำเร็จ'); document.getElementById('inputDutySport').value = ''; document.getElementById('inputDutyRole').value = ''; document.getElementById('inputDutyEvent').value = ''; document.getElementById('inputDutyYear').value = ''; fetchData(); } else { alert(`❌ ข้อผิดพลาด: ${result.message}`); }
  } catch(e) { alert('❌ การเชื่อมต่อล้มเหลว'); }
...
function handleCascadingFilter(changedType) {
  ...
  const yearSelect = document.getElementById('filterYear'); const courseSelect = document.getElementById('filterCourse'); const selectedYear = yearSelect.value; const selectedCourse = courseSelect.value; const relations = globalFiltersMaster.relations;
...
function updateDropdownUI() {
  ...
  const relations = globalFiltersMaster.relations; let availableYears = globalFiltersMaster.years; let availableCourses = globalFiltersMaster.courses; let availableGroups = globalFiltersMaster.groups;
  ...
  populateDropdown('filterYear', availableYears, selectedYear, 'ทุกปีการศึกษา'); populateDropdown('filterCourse', availableCourses, selectedCourse, 'ทุกหลักสูตร'); populateDropdown('filterGroup', availableGroups, selectedGroup, 'ทุกกลุ่มบุคลากร');
}
...
window.exportToExcel = function() {
  ...
  currentFilteredData.forEach(user => {
    let userTrainings = user.trainings || []; let matchedTrainings = userTrainings.filter(t => { const matchY = filterYear === '' || String(t.year) === String(filterYear); const matchC = filterCourse === '' || String(t.course) === String(filterCourse); return matchY && matchC; });
    if (matchedTrainings.length > 0) { matchedTrainings.forEach(t => { exportData.push({ 'รหัส UID': user.uid, 'ชื่อ-นามสกุล': user.fullName, 'กลุ่มหน่วยงาน': user.group || '-', 'หน่วยงาน': user.agency, 'สถานะ': user.status, 'ชื่อหลักสูตร': t.course, 'ปีที่อบรม': parseInt(t.year) || t.year }); }); } 
    else if (filterYear === '' && filterCourse === '') { exportData.push({ 'รหัส UID': user.uid, 'ชื่อ-นามสกุล': user.fullName, 'กลุ่มหน่วยงาน': user.group || '-', 'หน่วยงาน': user.agency, 'สถานะ': user.status, 'ชื่อหลักสูตร': '-', 'ปีที่อบรม': '-' }); }
  });
