...
function renderTimeline(relations, years, courses) {
  ...
  container.innerHTML = courses.map((course, idx) => {
    ...
    let missingYears = [];
    for (let y = firstYear; y <= lastYear; y++) { if (!activeYearsMap[y]) missingYears.push(y); }
    return `
      <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow space-y-5">
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            ${activeYears.map(y => `
              <div class="flex items-center gap-1.5 bg-blue-600 text-white text-xs font-bold px-3.5 py-1.5 rounded-xl shadow-sm"><svg class="w-3.5 h-3.5 text-blue-200" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg> ปี ${y}</div>
            `).join('')}
          </div>
        </div>
        ...
        ` : `<div class="pt-3 border-t border-slate-100 text-xs text-emerald-600 font-semibold flex items-center gap-1.5"><span>✨ จัดอบรมต่อเนื่องทุกปีโดยไม่มีช่วงว่าง (Continuous Lifecycle)</span></div>`}
      </div>
    `;
  }).join('');
}
  ...
  reader.onload = function(event) {
    try {
      ...
      let cleanedRows = jsonRows.map((row, index) => {
        ...
        if (exactFound) {
          matchedExisting = exactFound; matchType = 'exact';
        } else {
...
function renderPreviewTablePage() {
  ...
  tbody.innerHTML = pageData.map((row, idx) => {
    ...
    if (row.matchType === 'exact') {
      badgeHtml = `<span class="bg-blue-50 text-blue-700 border border-blue-200 px-2.5 py-1 rounded-full text-xs font-semibold">🔄 รวมกับคนเดิม (${row.matchedUser.fullName})</span>`;
...
window.updateImportAction = function(absoluteIndex, choice) {
  if (pendingImportData[absoluteIndex]) { pendingImportData[absoluteIndex].actionType = choice; }
}
...
window.changePreviewPage = function(newPage) {
  ...
  if (newPage >= 1 && newPage <= totalPages) { currentPreviewPage = newPage; renderPreviewTablePage(); }
}
...
window.confirmImport = async function() {
  ...
  try {
    const response = await fetch(API_URL, {
      method: 'POST', body: JSON.stringify({ action: 'bulkImport', rows: processedRows }),
      headers: { 'Content-Type': 'text/plain;charset=utf-8' }
    });
    const result = await response.json();
    if (result.status === 'success') { 
      alert(`✅ ${result.message}`); globalFiltersMaster = null; fetchData(); cancelImport(); switchPage('search'); 
...
window.viewProfile = function(uid) {
  ...
  setTimeout(() => { backdrop.classList.remove('opacity-0'); backdrop.classList.add('opacity-100'); panel.classList.remove('translate-x-full'); panel.classList.add('translate-x-0'); }, 10);
  switchTab('general');
}
...
window.exportToExcel = function() {
  const filterYear = document.getElementById('filterYear').value; const filterCourse = document.getElementById('filterCourse').value;
  let exportData = [];
