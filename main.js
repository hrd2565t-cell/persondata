...
// 📌 ฟังก์ชันพิมพ์บัตรประจำตัว (ดึง API generatePDF)
window.generateIdCard = async function(uid, fullName, course, year, sport, role, btnElement) {
  const originalHtml = btnElement.innerHTML;
  btnElement.innerHTML = `<svg class="animate-spin w-4 h-4 mr-1.5" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> กำลังสร้าง...`;
  btnElement.disabled = true;
  
  try {
    const payload = { action: 'generatePDF', uid, fullName, course, year, sport, role };
    const response = await fetch(API_URL, { method: 'POST', body: JSON.stringify(payload), headers: { 'Content-Type': 'text/plain;charset=utf-8' } });
    const result = await response.json();
    ...
    if (result.status === 'success') {
      ...
      showToast('ดาวน์โหลดเอกสารสำเร็จ');
    } else {
...
window.viewProfile = function(uid) {
  ...
  if (person.duties && person.duties.length > 0) {
    dutyEl.innerHTML = person.duties.map((d, dIdx) => {
      let evidenceHtml = '';
      if (d.images) {
        ...
        if (links.length > 0) {
          ...
          links.forEach((link, idx) => {
            let label = idx === 0 ? "ภาพประจำตัว" : `หลักฐานชิ้นที่ ${idx}`;
            evidenceHtml += `
      ...
      let editBtnHtml = `
        <button onclick="editDutyRecord('${d.rowIndex}', '${d.sport}', '${d.role}', '${d.event}', '${d.year}')" class="text-[11px] font-bold text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 px-2.5 py-1.5 rounded-md transition flex items-center gap-1 w-full justify-center">
          ✏️ แก้ไขข้อมูล
        </button>`;
        
      let generateIdBtnHtml = `
        <button onclick="generateIdCard('${person.uid}', '${person.fullName}', '${d.course}', '${d.year}', '${d.sport}', '${d.role}', this)" class="text-[11px] font-bold text-white bg-blue-600 hover:bg-blue-700 px-2.5 py-1.5 rounded-md transition flex items-center gap-1 w-full justify-center shadow-sm">
          🪪 พิมพ์บัตรประจำตัว
        </button>`;

      return `
      <li class="bg-white p-3.5 rounded-xl border border-slate-200 flex flex-col gap-1 shadow-sm relative">
        <div class="flex flex-col gap-1.5 absolute top-3 right-3 w-28">
           ${editBtnHtml}
           ${generateIdBtnHtml}
        </div>
        ...
        <span class="text-[10px] w-max font-bold bg-slate-100 text-slate-600 px-2 py-0.5 rounded mt-0.5">ปี ${d.year || '-'} | ${d.course}</span>
        <span class="text-xs text-blue-600 font-semibold mt-1">${d.role}</span>
...
window.loadProjectData = function(course) {
  ...
  document.getElementById('pdSec42').value = ''; 
  ...
  document.getElementById('pdLongTerm').value = ''; 
  ...
  document.getElementById('pdSkillImpact').value = ''; 
  ...
  if(cachedProjectDetails && cachedProjectDetails[course]) {
    ...
    document.getElementById('pdSec42').value = d.sec42 || ''; 
    ...
    document.getElementById('pdLongTerm').value = d.longTerm || ''; 
    ...
    document.getElementById('pdSkillImpact').value = d.skillImpact || ''; 
...
window.submitProjectDetails = async function() {
  ...
  if(!course || !document.getElementById('pdExpected').value.trim()) {
    return alert('⚠️ กรุณาเลือกหลักสูตรและระบุผลที่คาดว่าจะได้รับ (ช่องดอกจันสีแดง)');
  }
  ...
  const payload = { 
    ...
    targetDesc: document.getElementById('pdTargetDesc').value.trim(),
    sec42: document.getElementById('pdSec42').value.trim(),
    govPolicy: document.getElementById('pdGovPolicy').value.trim(),
    expected: document.getElementById('pdExpected').value.trim(), 
    shortTerm: document.getElementById('pdShortTerm').value.trim(),
    longTerm: document.getElementById('pdLongTerm').value.trim(),
    socialImpact: document.getElementById('pdSocialImpact').value.trim(),
    envImpact: document.getElementById('pdEnvImpact').value.trim(),
    healthImpact: document.getElementById('pdHealthImpact').value.trim(),
    skillImpact: document.getElementById('pdSkillImpact').value.trim(),
...
window.saveAdminPin = async function() {
  ...
  if(!pinInput || pinInput.length !== 6 || isNaN(pinInput)) return alert('⚠️ รหัสต้องเป็นตัวเลข 6 หลัก');
  ...
  try {
    ...
    const result = await response.json();
    if(result.status === 'success') {
      alert('✅ เปลี่ยนรหัสสำเร็จ'); globalSettings.adminPin = pinInput; document.getElementById('adminPinSetting').value = ''; 
    } else { alert('❌ ข้อผิดพลาด: ' + result.message); }
  } catch(e) { alert('❌ การเชื่อมต่อล้มเหลว'); }
...
window.exportToExcel = function() {
  ...
  currentFilteredData.forEach(user => {
    let matchedTrainings = (user.trainings || []).filter(t => (filterYear === '' || String(t.year) === String(filterYear)) && (filterCourse === '' || String(t.course) === String(filterCourse)));
  ...
  exportData.sort((a, b) => (parseInt(a['ปีที่อบรม']) || 9999) - (parseInt(b['ปีที่อบรม']) || 9999));
...
// 📌 ฟังก์ชันจัดการปุ่มลบรูปภาพเดิมออกทีละรูป
window.removeOldImage = function(formIdx, imgType) {
  if(confirm('🗑️ ยืนยันการลบรูปภาพนี้?\n(ไฟล์จะถูกทำลายทิ้งจาก Google Drive ทันทีหลังกดปุ่มบันทึกข้อมูล)')) {
    document.getElementById(`keepImg${imgType}_${formIdx}`).value = '';
    const box = document.getElementById(`oldImgBox_${formIdx}_${imgType}`);
    if(box) box.style.display = 'none';
  }
};

// 📌 ฟังก์ชันโชว์ Preview ตอนผู้ใช้เลือกรูปใหม่จากคอม
...
window.renderSrForms = function() {
  ...
  uniqueCourses.forEach((courseName) => {
    ...
    formsForCourse.forEach((form, relativeIdx) => {
      ...
      let recordIdVal = d.recordId || ''; 
      
      // 📌 ระบบจัดการโชว์ภาพ Preview 3 ช่อง
      let keepImg1 = '', keepImg2 = '', keepImg3 = '';
      if (form.data && form.data.images) {
         let arr = form.data.images.split(',').map(s=>s.trim());
         keepImg1 = arr[0] || '';
         keepImg2 = arr[1] || '';
         keepImg3 = arr[2] || '';
      }

      let oldImg1Html = keepImg1 ? `
         <div id="oldImgBox_${i}_1" class="relative inline-block border border-slate-200 rounded p-1 bg-white mb-2 shadow-sm w-max">
           <img src="${keepImg1}" class="h-16 w-auto object-cover rounded">
           <button type="button" onclick="removeOldImage(${i}, 1)" class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center shadow-md transition-colors" title="ลบรูปภาพนี้">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
           </button>
         </div>` : '';
         
      let oldImg2Html = keepImg2 ? `
         <div id="oldImgBox_${i}_2" class="relative inline-block border border-slate-200 rounded p-1 bg-white mb-2 shadow-sm w-max">
           <img src="${keepImg2}" class="h-16 w-auto object-cover rounded">
           <button type="button" onclick="removeOldImage(${i}, 2)" class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center shadow-md transition-colors" title="ลบรูปภาพนี้">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
           </button>
         </div>` : '';
         
      let oldImg3Html = keepImg3 ? `
         <div id="oldImgBox_${i}_3" class="relative inline-block border border-slate-200 rounded p-1 bg-white mb-2 shadow-sm w-max">
           <img src="${keepImg3}" class="h-16 w-auto object-cover rounded">
           <button type="button" onclick="removeOldImage(${i}, 3)" class="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-5 h-5 flex items-center justify-center shadow-md transition-colors" title="ลบรูปภาพนี้">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
           </button>
         </div>` : '';
      ...
      <div class="sr-card-item bg-white border border-slate-200 rounded-2xl p-6 md:p-8 relative shadow-sm mb-5">
        ...
        <input type="hidden" id="srRecordId_${i}" value="${recordIdVal}">
        <input type="hidden" id="keepImg1_${i}" value="${keepImg1}">
        <input type="hidden" id="keepImg2_${i}" value="${keepImg2}">
        <input type="hidden" id="keepImg3_${i}" value="${keepImg3}">
        ...
        <div class="flex justify-between items-center mb-5 mt-2">
           ...
           <button type="button" onclick="removeDutyForm(${i})" class="text-xs font-bold text-red-500 hover:text-red-700 bg-red-50 px-3 py-1 rounded-lg transition">ลบฟอร์มงานนี้</button>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="bg-blue-50/50 border border-blue-100 p-4 rounded-xl">
            <label class="block text-xs font-bold text-slate-700 mb-3 border-b border-blue-200 pb-2">อัปโหลดรูปภาพประกอบ (รองรับ 3 ช่องทาง / ไม่เกิน 10MB ต่อภาพ)</label>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
              
              <div class="flex flex-col bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                 <span class="text-xs font-bold text-slate-600 mb-2 flex items-center gap-1.5">👤 1. ภาพประจำตัว</span>
                 ${oldImg1Html}
                 <input type="file" id="srFile1_${i}" accept="image/jpeg, image/png, image/jpg" onchange="previewImage(this, 'preview1_${i}')" class="w-full text-[10px] text-slate-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                 <div id="preview1_${i}_container" class="hidden mt-2 bg-slate-50 rounded border border-slate-200 p-1 flex justify-center items-center h-20 shadow-inner">
                     <img id="preview1_${i}" src="" class="hidden max-h-full max-w-full object-contain rounded">
                 </div>
              </div>
              
              <div class="flex flex-col bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                 <span class="text-xs font-bold text-slate-600 mb-2 flex items-center gap-1.5">📷 2. หลักฐานชิ้นที่ 1</span>
                 ${oldImg2Html}
                 <input type="file" id="srFile2_${i}" accept="image/jpeg, image/png, image/jpg" onchange="previewImage(this, 'preview2_${i}')" class="w-full text-[10px] text-slate-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                 <div id="preview2_${i}_container" class="hidden mt-2 bg-slate-50 rounded border border-slate-200 p-1 flex justify-center items-center h-20 shadow-inner">
                     <img id="preview2_${i}" src="" class="hidden max-h-full max-w-full object-contain rounded">
                 </div>
              </div>
              
              <div class="flex flex-col bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                 <span class="text-xs font-bold text-slate-600 mb-2 flex items-center gap-1.5">📷 3. หลักฐานชิ้นที่ 2</span>
                 ${oldImg3Html}
                 <input type="file" id="srFile3_${i}" accept="image/jpeg, image/png, image/jpg" onchange="previewImage(this, 'preview3_${i}')" class="w-full text-[10px] text-slate-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer">
                 <div id="preview3_${i}_container" class="hidden mt-2 bg-slate-50 rounded border border-slate-200 p-1 flex justify-center items-center h-20 shadow-inner">
...
// 📌 ฟังก์ชันบีบอัดรูปลดขนาดไฟล์อัตโนมัติ 
...
window.submitSelfReport = async function() {
  ...
  for (let i = 0; i < srFormState.length; i++) {
    ...
    if (recordId) keptIds.push(recordId);
  ...
  try {
    ...
    // แจ้งหลังบ้านว่าอันไหนถูกลบออกไปบ้าง
    document.getElementById('srLoadingTitle').textContent = `กำลังซิงโครไนซ์ข้อมูลกับระบบ...`;
    await fetch(API_URL, { 
       method: 'POST', 
       body: JSON.stringify({ action: 'deleteMissingDutyRecords', uid: srSelectedUser.uid, year: activeYear, keptRecordIds: keptIds }),
    ...
    for (let i = 0; i < allReports.length; i++) {
      ...
      const payload = { 
        ...
        keepImg2: r.keepImg2, 
        keepImg3: r.keepImg3, 
        file1Data: r.file1Data || null, file1Name: r.file1Name || '', file1Mime: r.file1Mime || '', 
        file2Data: r.file2Data || null, file2Name: r.file2Name || '', file2Mime: r.file2Mime || '',
        file3Data: r.file3Data || null, file3Name: r.file3Name || '', file3Mime: r.file3Mime || ''
      };
    ...
    alert('✅ บันทึกรายงานสำเร็จ ข้อมูลอัปเดตเรียบร้อยแล้ว'); 
    document.getElementById('srSearchName').value = ''; 
    document.getElementById('srFormContainer').classList.add('hidden'); 
    srSelectedUser = null; 
    srFormState = [];
    ...
    if(err.message === 'ขนาดไฟล์ต้องไม่เกิน 10MB') alert('❌ ' + err.message); 
    else if(err.message.includes('ระบบมีผู้ใช้งาน')) alert('⚠️ ' + err.message);
